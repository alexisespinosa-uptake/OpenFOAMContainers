...
cleanCFDEMcase()
{
    ...
    read
    #AEG:source $WM_PROJECT_DIR/bin/tools/CleanFunctions
    #CFD
    cd $casePath/CFD
...
parDEMrun()
{
    ...
    if [[ $machineFileName == "none" ]]; then
        #AEG:mpirun -np $nrProcs $debugMode $CFDEM_LIGGGHTS_EXEC -in $solverName 2>&1 | tee -a $logpath/$logfileName
        srun --export=all -n $nrProcs shifter run --mpi $containerImage $CFDEM_LIGGGHTS_EXEC -in $solverName 2>&1 | tee -a $logpath/$logfileName
    else
        #AEG:mpirun -machinefile $machineFileName -np $nrProcs $debugMode $CFDEM_LIGGGHTS_EXEC -in $solverName 2>&1 | tee -a $logpath/$logfileName
        srun --export=all -n $nrProcs shifter run --mpi $containerImage $CFDEM_LIGGGHTS_EXEC -in $solverName 2>&1 | tee -a $logpath/$logfileName
    fi
...
parCFDrun()
{
    ...
    #- decompose case
    #AEG:decomposePar
    shifter run $containerImage decomposePar
    ...
    #- run applictaion
    if [[ $machineFileName == "none" ]]; then
        #AEG:mpirun -np $nrProcs $debugMode $solverName -parallel 2>&1 | tee -a $logpath/$logfileName
        srun --export=all -n $nrProcs shifter run -mpi $containerImage $solverName -parallel 2>&1 | tee -a $logpath/$logfileName
    else
        #AEG:mpirun -machinefile $machineFileName -np $nrProcs $debugMode $solverName -parallel 2>&1 | tee -a $logpath/$logfileName
        srun --export=all -n $nrProcs shifter run --mpi $containerImage $solverName -parallel 2>&1 | tee -a $logpath/$logfileName
    fi
...
parCFDDEMrun()
{
    ...
    if [[ $decomposeCase == "false" ]]; then   
        ...
        #AEG:decomposePar -force
        shifter run $containerImage decomposePar -force
    ...
    if [[ $machineFileName == "none" ]]; then
        #AEG:mpirun -np $nrProcs $debugMode $solverName -parallel 2>&1 | tee -a $logpath/$logfileName
        echo "ALEXIS:"
        echo "srun --export=all -n $nrProcs shifter run $containerImage $solverName -parallel 2>&1 | tee -a $logpath/$logfileName"
        srun --export=all -n $nrProcs shifter run --mpi $containerImage $solverName -parallel 2>&1 | tee -a $logpath/$logfileName

        #- reconstruct case
        if [[ $reconstuctCase == "true" ]]; then   
            #pseudoParallelRun "reconstructPar" $nrProcs
            #AEG:reconstructPar -noLagrangian
            shifter run $containerImage reconstructPar -noLagrangian
        fi
    else
        #AEG:mpirun -machinefile $machineFileName -np $nrProcs $debugMode $solverName -parallel 2>&1 | tee -a $logpath/$logfileName
        srun --export=all -n $nrProcs shifter run --mpi $containerImage $solverName -parallel 2>&1 | tee -a $logpath/$logfileName
    ...
    if [[ $debugMode == "hpcrun" ]]; then
        if [ -f $CFDEM_APP_DIR/$solverName.hpcstruct ]; then
            ...
            #AEG:hpcprof -S $CFDEM_APP_DIR/$solverName.hpcstruct -I ./'*' hpctoolkit-$solverName-measurements
            shifter run $containerImage hpcprof -S $CFDEM_APP_DIR/$solverName.hpcstruct -I ./'*' hpctoolkit-$solverName-measurements
        else
